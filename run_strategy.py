...
def generate_orders(df, ind, htf_trend, params):
    ...
    # ТОРГОВЛЯ ТОЛЬКО ПО ТРЕНДУ (строгий режим)
    # Лонг: только если бычий тренд (htf_trend == 1) + откат к нижней границе BB или низкий RSI
    # Шорт: только если медвежий тренд (htf_trend == -1) + отскок от верхней границы BB или высокий RSI
    
    # Покупка: бычий тренд + (цена <= нижняя граница BB И RSI < 40)
    entry_long = (htf_trend == 1) & (c <= ind['bb_lower']) & (ind['rsi'] < 40)
    
    # Продажа: медвежий тренд + (цена >= верхняя граница BB И RSI > 60)
    entry_short = (htf_trend == -1) & (c >= ind['bb_upper']) & (ind['rsi'] > 60)
    ...
    # Используем direction=2 (Both) для всех ордеров - это позволяет работать с long/short
    # size=NaN означает отсутствие ордера
    final_directions = np.full(n, 2, dtype=np.int32)  # По умолчанию Both (2)
    final_sizes = np.full(n, np.nan, dtype=np.float64)  # По умолчанию нет ордера
    
    for i in range(n):
        if long_entries[i]:
            final_directions[i] = 2  # Both - вход в лонг (положительный размер)
            final_sizes[i] = sizes[i]
        elif short_entries[i]:
            final_directions[i] = 2  # Both - вход в шорт (отрицательный размер через знак)
            final_sizes[i] = -sizes[i]  # Отрицательный размер для шорта
        elif long_exits[i]:
            final_directions[i] = 2  # Both - выход из лонга (продажа всей позиции)
            final_sizes[i] = -1.0  # Закрываем всю позицию (100%)
        elif short_exits[i]:
            final_directions[i] = 2  # Both - выход из шорта (покупка для закрытия)
            final_sizes[i] = 1.0  # Закрываем шорт (покупаем обратно)
...
# Оптимизированная сетка параметров (сфокусирована на лучших диапазонах)
param_grid = {
    'confluence_thresh': [2, 3],           # Порог конфлюэнции сигналов
    'vol_ratio_thresh': [1.0, 1.5],        # Порог объема
    'sl_atr_mult': [2.0, 2.5],             # Стоп-лосс в ATR (оптимально 2-2.5)
    'tp_atr_mult': [3.0, 4.0],             # Тейк-профит в ATR (оптимально 3-4)
    'avg_atr_mult': [2.0],                 # Усреднение по ATR
...
for vals in product(*param_grid.values()):
    ...
    try:
        ...
        # Проверка: direction должен быть 2 (Both), size может быть положительным или отрицательным
        active_mask = ~np.isnan(sizes)
        if np.any(active_mask):
            assert np.all(directions[active_mask] == 2), f"direction должен быть 2 (Both), найдено: {np.unique(directions[active_mask])}"
            # Размеры могут быть любыми (положительные для лонга, отрицательные для шорта)
