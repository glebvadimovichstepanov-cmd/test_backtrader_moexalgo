...
def load_data():
    ...
    # Защита от 0/NaN в ценах (деление на 0 ломает кэш в vbt)
...
# ──────────────────────────────────────────────
# 📤 2. ГЕНЕРАЦИЯ ОРДЕРОВ (ПЕРЕПИСАНО ПО ВАШИМ ПРАВИЛАМ)
# ──────────────────────────────────────────────
def generate_orders(df, ind, htf_trend, params):
    ...
    # 🔑 Инициализация под VBT 1.0.0:
    # direction = 1 (по умолчанию, VBT требует только 1 или -1)
    # size = NaN (пропуск бара, VBT не исполняет ордер при NaN)
    ...
    for i in range(n):
        if cooldown > 0:
            cooldown -= 1
            continue  # size остаётся NaN, direction=1 (игнорируется движком)
        ...
        if pos == 0:
            ...
            if entry_long.iloc[i]:
                ...
                tp_price = px - p['tp_atr_mult'] * atr_i
                avg_done = False
                directions[i] = -1  # Шорт вход
                sizes[i] = 1.0

        elif pos == 1:
            # УСРЕДНЕНИЕ ЛОНГА (направление совпадает с позицией)
            if not avg_done and px <= entry_price - p['avg_atr_mult'] * atr_i:
                ...
                directions[i] = 1
                sizes[i] = p['avg_size']
            # ЗАКРЫТИЕ ЛОНГА (направление противоположно позиции)
            ...
            # УСРЕДНЕНИЕ ШОРТА (направление совпадает с позицией)
            if not avg_done and px >= entry_price + p['avg_atr_mult'] * atr_i:
                entry_price = (entry_price + px * p['avg_size']) / (1 + p['avg_size'])
                sl_price = entry_price + p['sl_atr_mult'] * atr_i
                tp_price = entry_price - p['tp_atr_mult'] * atr_i
                ...
                directions[i] = -1
                sizes[i] = p['avg_size']
            # ЗАКРЫТИЕ ШОРТА (направление противоположно позиции)
...
for vals in product(*param_grid.values()):
    ...
    try:
        ...
        # ✅ ВАЛИДАЦИЯ ПЕРЕД VBT (строгое соответствие требованиям)
        assert np.all(np.isin(directions, [1, -1])), "direction содержит недопустимые значения (только 1 или -1)"
        active_mask = ~np.isnan(sizes)
        if np.any(active_mask):
            assert np.all(sizes[active_mask] > 0), "Активные размеры ордеров должны быть > 0"

        # Принудительная C-континуальность для Numba
        ...
        if DEBUG:
            print(f"\n🔍 Конфиг #{cfg_count}: {p}")
            print(f"  📊 direction unique: {np.unique(dirs_arr)} | active orders: {active_mask.sum()}")
        ...
        pf = vbt.Portfolio.from_orders(
            ...
            cash_sharing=False,  # Изолирует кэш
            lock_cash=False
        )
